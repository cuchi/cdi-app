...
function inviteStudent(classroomId, { email }, user) {
    assertTeacher(user);

    if (!email) throw new ValidationError('E-mail required!');

    return Teacher.findById(user._id)
        .then(teacher => createInvite(classroomId, email, teacher.name));
}
